...
```
Obtenemos tres términos para cada región. El primero, \code{M}, coincide con los valores totales sin la opción \code{within}. El segundo, \code{M\_B\_csep}, conocido como término "between", es la segregación socioeconómica y étnica en las combinaciones de escuelas y tipos de escuelas. El tercero, \code{M\_W\_csep}, conocido como término "within", es la combinacion lineal de los niveles de segregacion étnica obtenidos dentro de cada nivel socioeconomico (con pesos iguales a la importancia demográfica de cada nivel). Este termino "within" se puede interpretar como la parte de la segregacion total derivada exclusivamente de diferencias etnicas. A partir de este punto nos referiremos a esta segregacion derivada exclusivamente de diferencias etnicas como "la contribucion de etnia".

También es posible obtener la descomposición del índice en un término "between" etnia y un término "within" etnia"

```{r}
mutual(data = DT_Seg_Chile, group = c("csep", "ethnicity"), unit = "school",by = "region", within = "ethnicity")
```
